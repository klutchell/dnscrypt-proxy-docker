...
manifest: ## Create and push a multiarch manifest to the docker repo (requires docker login)
	...
	docker manifest push --purge ${DOCKER_REPO}:${TAG}
	-docker manifest push --purge ${DOCKER_REPO}:beta
	docker manifest create ${DOCKER_REPO}:beta \
		...
		${DOCKER_REPO}:i386-${TAG} \
		${DOCKER_REPO}:ppc64le-${TAG}
	docker manifest annotate ${DOCKER_REPO}:beta ${DOCKER_REPO}:amd64-${TAG} --os linux --arch amd64
	docker manifest annotate ${DOCKER_REPO}:beta ${DOCKER_REPO}:arm32v6-${TAG} --os linux --arch arm --variant v6
	docker manifest annotate ${DOCKER_REPO}:beta ${DOCKER_REPO}:arm32v7-${TAG} --os linux --arch arm --variant v7
	docker manifest annotate ${DOCKER_REPO}:beta ${DOCKER_REPO}:arm64v8-${TAG} --os linux --arch arm64 --variant v8
	docker manifest annotate ${DOCKER_REPO}:beta ${DOCKER_REPO}:i386-${TAG} --os linux --arch 386
	docker manifest annotate ${DOCKER_REPO}:beta ${DOCKER_REPO}:ppc64le-${TAG} --os linux --arch ppc64le
	docker manifest push --purge ${DOCKER_REPO}:beta
